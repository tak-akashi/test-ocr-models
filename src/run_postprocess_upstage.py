#!/usr/bin/env python3
"""
Post-processing script for upstage-ocr results.
Compares upstage-ocr output with ground truth dataset and generates evaluation reports.
"""
...
def extract_text_from_html(html_content: str) -> str:
    """
    Extract text from Upstage HTML output.
...
def extract_text_from_file(file_path: Path) -> str:
    """
    Extract text from HTML or Markdown file.

    Args:
        file_path: Path to HTML or Markdown file
...
def process_upstage_outputs(
    upstage_dir: Path,
    gt_lookup: dict[str, str]
) -> list[dict[str, Any]]:
    """
    Process all upstage-ocr output files.

    Args:
        upstage_dir: Directory containing upstage-ocr HTML/Markdown outputs
    ...
    output_files = sorted(list(upstage_dir.rglob("*.html")) + list(upstage_dir.rglob("*.md")))
    ...
    for output_file in output_files:
        ...
        result = {
            ...
            "output_file": str(output_file.relative_to(upstage_dir)),
...
<head>
    ...
    <title>UPSTAGE-OCR Evaluation Results</title>
    <style>
        body {{
...
<body>
    <h1>UPSTAGE-OCR Evaluation Results</h1>
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Post-process upstage-ocr results and compare with ground truth"
    )
    parser.add_argument(
        "upstage_dir",
        type=Path,
        help="Path to upstage-ocr output directory (e.g., output/20250125-1430/upstage-ocr/)"
    )
    parser.add_argument(
        "--gt-dataset",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output directory for results (default: same as upstage_dir with _evaluation suffix)"
    )

    args = parser.parse_args()

    # Validate inputs
    if not args.upstage_dir.exists():
        print(f"Error: upstage-ocr directory not found: {args.upstage_dir}", file=sys.stderr)
        sys.exit(1)
    ...
    if args.output_dir is None:
        args.output_dir = args.upstage_dir.parent / f"{args.upstage_dir.name}_evaluation"

    args.output_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 80)
    print("UPSTAGE-OCR Post-Processing")
    print("=" * 80)
    print(f"Input directory: {args.upstage_dir}")
    ...
    # Process upstage outputs
    print("Processing upstage-ocr outputs...")
    results = process_upstage_outputs(args.upstage_dir, gt_lookup)
