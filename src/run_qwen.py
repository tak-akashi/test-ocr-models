"""Main script for running Qwen model tests."""
...
def run_qwen_timed_with_datetime(file_list, base_output_dir=None):
    ...
    for file_idx, file_path in enumerate(file_list):
        ...
        try:
            _, exec_time = measure_time(run_qwen3vl_optimized, file_path,
                                       output_dir=base_output_dir / "qwen3vl", save=True)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run Qwen model tests")
    ...
    parser.add_argument("--include-qwen3", action="store_true",
                       help="Include Qwen3VL model (experimental)")
